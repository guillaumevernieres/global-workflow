...
class GFSTasks(Tasks):
    ...
    def jedicoupledvarinit(self):
        """
        JEDI coupled variational DA initialization task.
        Combines atmospheric and marine analysis initialization for coupled atmosphere-ocean-seaice DA.
        """
        ...
        # Marine background error covariance
        dep_dict = {'type': 'task', 'name': f'{self.run}_marinebmat'}
        deps.append(rocoto.add_dependency(dep_dict))

        # Previous forecast for background
        dep_dict = {'type': 'metatask', 'name': 'gdas_fcst', 'offset': f"-{timedelta_to_HMS(self._base['interval_gdas'])}"}
        deps.append(rocoto.add_dependency(dep_dict))

        # Ensemble if doing hybrid variational
        if self.options['do_hybvar']:
            dep_dict = {'type': 'metatask', 'name': 'enkfgdas_epmn', 'offset': f"-{timedelta_to_HMS(self._base['interval_gdas'])}"}
            deps.append(rocoto.add_dependency(dep_dict))
        ...
        resources = self.get_resource('jedicoupledvarinit')
        task_name = f'{self.run}_jedicoupledvarinit'
        task_dict = {'task_name': task_name,
                     'resources': resources,
                     'dependency': dependencies,
                     'envars': self.envars,
                     'cycledef': self.run.replace('enkf', ''),
                     'command': f'{self.HOMEgfs}/dev/job_cards/rocoto/jedicoupledvarinit.sh',
                     'job_name': f'{self.pslot}_{task_name}_@H',
                     'log': f'{self.rotdir}/logs/@Y@m@d@H/{task_name}.log',
                     'maxtries': '&MAXTRIES;'
                     }
